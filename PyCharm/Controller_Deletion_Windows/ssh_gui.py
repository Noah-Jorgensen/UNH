...
def delete():
    if selectedVarLabel2.cget("text") != "NONE SELECTED":
        if selectedTypeLabel2.cget("text") == "Username":
            messagebox.showinfo("WARNING", "WARNING: Deleting a username will remove ALL OF THE CLIENT'S DEVICES that "
                                           "are currently connected!", icon='warning')
...
mainWindow.geometry("400x400")  # width x height
...
enterLabel = tkinter.Label(tab1, text="Enter username/mac/ip for deletion: ")
...
deletionEntry = tkinter.Entry(tab1, textvariable=deletionVar, width=20)
deletionEntry.place(x=227, y=12)

typeLabel = tkinter.Label(tab1, text="Choose type: ")
typeLabel.place(x=150, y=45)
...
dropdown.place(x=226, y=41)

# SELECT ITEM
selectButton = tkinter.Button(tab1, text="SELECT", width=20, command=select)
selectButton.place(x=30, y=100)

# SELECTED AREA
selectedVarLabel = tkinter.Label(tab1, text="Selected to delete: ")
selectedVarLabel.place(x=121, y=160)
selectedVarLabel2 = tkinter.Label(tab1, text="NONE SELECTED", anchor="w", fg="white", width=17, bg="red")
selectedVarLabel2.place(x=227, y=160)

selectedTypeLabel = tkinter.Label(tab1, text="Selected type: ")
selectedTypeLabel.place(x=143, y=190)
selectedTypeLabel2 = tkinter.Label(tab1, text=dropOption.get(), anchor="w", fg="white", width=17, bg="red")
selectedTypeLabel2.place(x=227, y=190)

# DELETE ITEM
deleteButton = tkinter.Button(tab1, text="DELETE", width=20, command=delete)
...
# CHOOSE FILE BUTTON
fileButton = tkinter.Button(tab2, text="Choose MAC Address CSV File", width=25, command=ask_for_file)
fileButton.place(x=30, y=20)

# CSV ENTRY
csvVar = tkinter.StringVar(mainWindow)
csvVar.set("")  # default value (do not change this one)
csvEntry = tkinter.Entry(tab2, textvariable=csvVar, width=55)
csvEntry.place(x=30, y=60)

# SELECT CSV
mac_list_master = []
csv_selectButton = tkinter.Button(tab2, text="SELECT THIS FILE", width=20, command=fill_mac_list)
csv_selectButton.place(x=30, y=90)
...
selectedCSVLabel = tkinter.Label(tab2, text="Selected: ")
selectedCSVLabel.place(x=30, y=130)
selectedCSVLabel2 = tkinter.Label(tab2, text="NONE SELECTED", anchor="w", fg="white", width=17, bg="red")
selectedCSVLabel2.place(x=90, y=130)
broken_macs = []

# DELETE ITEM CSV
csv_deleteButton = tkinter.Button(tab2, text="DELETE ALL MAC ENTRIES", width=25, command=delete_csv)
csv_deleteButton.place(x=30, y=165)
deleted_list = []
not_deleted_list = []
...
hostnameLabel.config(text="Enter hostname to add or remove:")
hostnameLabel.place(x=30, y=40)

# HOSTNAME ENTRY
hostnameVar = tkinter.StringVar(mainWindow)
hostnameVar.set("")  # default value (do not change this one)
hostnameEntry = tkinter.Entry(tab3, textvariable=hostnameVar, width=55)
hostnameEntry.place(x=30, y=60)

# ADD HOSTNAME
hostname_add_Button = tkinter.Button(tab3, text="ADD HOSTNAME", width=20, command=hostname_add, bg="#50FB70")
hostname_add_Button.place(x=40, y=100)

# REMOVE HOSTNAME
hostname_remove_Button = tkinter.Button(tab3, text="REMOVE HOSTNAME", width=20, command=hostname_remove, bg="#FF4D4D")
hostname_remove_Button.place(x=200, y=100)

# ACTIVE HOSTNAMES LABEL
activeHostnamesLabel = tkinter.Label(tab3)
activeHostnamesLabel.config(text="ACTIVE HOSTNAME LIST:")
activeHostnamesLabel.place(x=30, y=150)

# ACTIVE HOSTNAMES LIST LABEL
activeHostnamesListLabel = tkinter.Label(tab3)
activeHostnamesListLabel.place(x=30, y=170)
...
try:
    ...
    no_fileLabel.config(text="'hostnames.txt' could not be found.")
    no_fileLabel.place(x=30, y=170)
